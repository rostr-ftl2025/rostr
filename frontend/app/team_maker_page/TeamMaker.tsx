...
// ---------------- API ----------------
async function fetchPitchers(params: { name?: string }) {
...
export default function TeamMakerPitchers() {
  ...
  useEffect(() => {
    const doFetch = async () => {
      ...
      setLoading(true);
      try {
        const data = await fetchPitchers({ name: debouncedName });
        setResults(data);
        if (data.length === 0) setError("No pitchers found for the given name.");
      } catch (e: any) {
  ...
  const addToRoster = (p: PitcherData) => {
    if (rosterIds.has(p.IDfg)) return;
